...
test('evaluarCasosPrueba aprueba cuando los 3 casos pasan en secuencia', async () => {
  let llamadas = 0;
  ...
  try {
    ...
    assert.equal(resultado.resultado, 'CUMPLE');
  } finally {
    restore();
  }
});

test('evaluarCasosPrueba detiene la secuencia cuando falla un caso logico', async () => {
  let llamadas = 0;
  const salidas = ['3', '999'];
  const { servicio, restore } = cargarServicioConMock(async () => {
    const salida = salidas[llamadas];
  ...
  try {
    ...
    assert.equal(resultado.resultado, 'NO CUMPLE');
    assert.equal(resultado.resultados[2].omitido, true);
  } finally {
    restore();
  }
});

test('evaluarCasosPrueba reporta error tecnico de Judge0 sin marcar incorrecto logicamente', async () => {
  let llamadas = 0;
  const { servicio, restore } = cargarServicioConMock(async () => {
    llamadas += 1;
    if (llamadas === 1) {
  ...
  try {
    ...
    assert.equal(resultado.errorTecnico, true);
    assert.equal(resultado.resultados[2].omitido, true);
  } finally {
    restore();
  }
...
test('evaluarCasosPrueba extrae el metodo cuando el estudiante envia una clase Main completa', async () => {
  ...
  try {
    ...
    assert.equal(resultado.cumple, true);
    assert.equal(llamadas, 3);
    assert.match(primerCodigoEnviado, /public class Main/);
    assert.match(primerCodigoEnviado, /public static int sumar\(int a, int b\)/);
    assert.doesNotMatch(primerCodigoEnviado, /int resultado = sumar\(a, b\);/);
  } finally {
    restore();
  }
});