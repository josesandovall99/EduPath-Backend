...
function chatbotResolveIncludes() {
  return [
    { association: 'Asignatura' },
    { association: 'miniproyecto' },
    { association: 'documentos' },
  ];
}

function chatbotFullIncludes({ orderDocumentosDesc = false } = {}) {
  const documentos = orderDocumentosDesc
    ? { association: 'documentos', separate: true, order: [['createdAt', 'DESC']] }
    : { association: 'documentos' };
  return [
    { association: 'Asignatura' },
    { association: 'miniproyecto' },
...
function sequelizeHttpDebug(error) {
  if (process.env.NODE_ENV === 'production' || !error) return {};
  const p = error.parent;
  if (!p) return {};
  const out = {};
  if (p.detail) out.pg_detail = p.detail;
  if (p.hint) out.pg_hint = p.hint;
  if (p.sqlMessage) out.sqlMessage = p.sqlMessage;
  return out;
}
...
function getTeacherasignaturaIds(req) {
  return Array.from(new Set((req.docenteAsignaturaIds || []).map((id) => Number(id)).filter((id) => Number.isFinite(id))));
}

function canManageAsignatura(req, asignaturaId) {
  if (isAdmin(req)) return true;
  return getTeacherasignaturaIds(req).includes(Number(asignaturaId));
}
...
function mapChatbotPayload(body) {
  ...
  return {
    ...
    asignatura_id: configuracion.asignatura_id ?? body.asignatura_id,
...
async function validateChatbotDependencies(req, payload, currentChatbot = null) {
  ...
  const asignaturaId = payload.asignatura_id !== undefined && payload.asignatura_id !== null ? Number(payload.asignatura_id) : (currentChatbot?.asignatura_id ?? null);
  ...
  if (isRoleScopedGeneralType(type)) {
    ...
    return { type, asignaturaId: null, miniproyectoId: null };
  }

  if (type === 'GENERAL') {
    if (!isAdmin(req) && !Number.isFinite(asignaturaId)) {
      return {
        error: {
          status: 400,
          payload: {
            mensaje: 'El chatbot general global del dashboard estudiantil solo puede ser gestionado por un administrador. Como docente debes asignar el chatbot general a una de tus asignaturas.',
          },
        },
      };
    }

    if (asignaturaId !== null && Number.isFinite(asignaturaId) && !canManageAsignatura(req, asignaturaId)) {
      return { error: { status: 403, payload: { mensaje: 'No tienes permisos para usar esa asignatura' } } };
    }

    if (asignaturaId !== null && Number.isFinite(asignaturaId)) {
      const asignatura = await AsignaturaModel.findByPk(asignaturaId);
      if (!asignatura) {
        return { error: { status: 404, payload: { mensaje: 'Asignatura no encontrada' } } };
      }
    }

    return { type, asignaturaId: Number.isFinite(asignaturaId) ? asignaturaId : null, miniproyectoId: null };
  }

  if (!Number.isFinite(asignaturaId)) {
    return { error: { status: 400, payload: { mensaje: 'asignatura_id es obligatorio para chatbots de tipo MINIPROYECTO' } } };
  }

  if (!canManageAsignatura(req, asignaturaId)) {
    return { error: { status: 403, payload: { mensaje: 'No tienes permisos para crear o modificar chatbots en esa asignatura' } } };
  }
  ...
  if (Number(miniproyecto.asignatura_id) !== asignaturaId) {
    return { error: { status: 400, payload: { mensaje: 'El miniproyecto seleccionado no pertenece al asignatura indicada' } } };
  }

  return { type, asignaturaId, miniproyectoId };
}

async function findManagedChatbot(req, chatbotId) {
  const chatbot = await Chatbot.findByPk(chatbotId, {
    include: chatbotFullIncludes({ orderDocumentosDesc: true }),
  });
  ...
  if (!isAdmin(req) && !Number.isFinite(Number(chatbot.asignatura_id))) {
    return { error: { status: 403, payload: { mensaje: 'Acceso denegado al chatbot solicitado' } } };
  }

  if (chatbot.asignatura_id && !canManageAsignatura(req, chatbot.asignatura_id)) {
    return { error: { status: 403, payload: { mensaje: 'Acceso denegado al chatbot solicitado' } } };
  }
  ...
  if (Number.isFinite(parsedasignaturaId)) {
    const AsignaturaGeneralChatbot = await Chatbot.findOne({
      where: {
        estado: true,
        tipo: 'GENERAL',
        asignatura_id: parsedasignaturaId,
      },
      include,
      order: [['updatedAt', 'DESC'], ['id', 'DESC']],
    });

    if (AsignaturaGeneralChatbot) {
      return AsignaturaGeneralChatbot;
    }
...
exports.resolveActiveChatbot = async (req, res) => {
  try {
    ...
    return res.json({
      ...
      tipo: chatbot.tipo,
      asignatura_id: chatbot.asignatura_id,
    ...
    return res.status(500).json({
      ...
      error: error.message,
      ...sequelizeHttpDebug(error),
    });
  }
};

function chatbotMatchesContext(chatbot, expectedType, expectedasignaturaId, expectedMiniproyectoId) {
  ...
  if (expectedType === 'GENERAL_DOCENTE' || expectedType === 'GENERAL_ADMINISTRADOR') {
    // Chatbots generales por rol son globales por diseño (sin asignatura/miniproyecto asociado).
    return true;
  }
  if (Number.isFinite(expectedasignaturaId) && Number(chatbot.asignatura_id) !== Number(expectedasignaturaId)) return false;
...
exports.createChatbot = async (req, res) => {
  try {
    ...
    const uniquenessError = await ensureSingleActiveGlobalGeneralChatbot({
      ...
      asignaturaId: dependency.asignaturaId,
    ...
    const chatbot = await Chatbot.create({
      ...
      asignatura_id: dependency.asignaturaId,
...
exports.getChatbots = async (req, res) => {
  try {
    const teacherasignaturaIds = getTeacherasignaturaIds(req);
    const where = isAdmin(req)
      ? {}
      : { asignatura_id: teacherasignaturaIds };
...
exports.uploadChatbotDocument = async (req, res) => {
  ...
  try {
    ...
    const fresh = await Chatbot.findByPk(chatbot.id, { include: [{ association: 'documentos' }] });
...
exports.chatWithManagedChatbot = async (req, res) => {
  try {
    const chatbot = await Chatbot.findByPk(req.params.id, { include: [{ association: 'documentos' }] });
