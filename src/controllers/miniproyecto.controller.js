...
const CONFIGURABLE_EXERCISE_TYPES = new Set(['Compilador', 'Diagramas UML', 'Preguntas', 'Opción única', 'Ordenar', 'Relacionar', 'Simulación GP']);
...
const evaluateEmbeddedExercise = async ({ exercise, reqBody }) => {
  ...
  if (exercise.tipo_ejercicio === 'Simulación GP') {
    const { evaluateSimulacionGp, extractSimulacionAnswer } = require('../utils/simulacionGp/evaluateSimulacionGp');
    const answer = extractSimulacionAnswer(reqBody);
    const result = evaluateSimulacionGp(
      { configuracion: exercise.configuracion, puntos: exercise.puntos },
      answer
    );
    return {
      status: result.esCorrecta ? 200 : 400,
      payload: {
        ejercicioId: exercise.id,
...
const validateConfigurableExercises = async (_asignaturaId, exerciseIds) => {
  ...
  const ejercicios = await Ejercicio.findAll({
    ...
    include: [
      { model: Actividad, as: 'actividad' },
      {
        model: Contenido,
        as: 'contenido',
        include: [{ model: Tema, attributes: ['asignatura_id', 'estado'] }],
      },
    ],
  });
...
const validateSelectedChatbot = async (asignaturaId, chatbotId, currentMiniproyectoId = null) => {
  ...
  if (Number(chatbot.asignatura_id) !== Number(asignaturaId)) {
    throw new Error('El chatbot seleccionado no pertenece al asignatura del miniproyecto.');
  }
...
const syncSelectedChatbot = async ({ asignaturaId, miniproyectoId, chatbotId, transaction }) => {
...
const validarRelaciones = async (tipo_id, asignatura_id) => {
  if (tipo_id) {
    const existe = await TipoActividad.findByPk(tipo_id);
    if (!existe) throw new Error(`El tipo_actividad_id (${tipo_id}) no existe.`);
  }
  if (asignatura_id) {
    const existe = await AsignaturaModel.findByPk(asignatura_id);
    if (!existe) throw new Error(`El asignatura_id (${asignatura_id}) no existe.`);
    if (existe.estado === false) throw new Error(`El asignatura_id (${asignatura_id}) está inactivo.`);
  }
};
...
const getPublishedMiniproyectoId = (AsignaturaLike) => {
  const parsed = Number(AsignaturaLike?.miniproyecto_publicado_id);
...
const serializeMiniproyectoSelection = (registro) => {
  ...
  const publishedId = getPublishedMiniproyectoId(payload?.Asignatura || payload?.Asignatura);
...
const filterStudentVisibleMiniproyectos = (items) => {
  const publishedByAsignatura = new Map();

  items.forEach((item) => {
    const payload = typeof item?.toJSON === 'function' ? item.toJSON() : item;
    const asignaturaId = Number(payload?.Asignatura?.id ?? payload?.asignatura_id);
    const publishedId = getPublishedMiniproyectoId(payload?.Asignatura || payload?.Asignatura);

    if (Number.isInteger(asignaturaId) && asignaturaId > 0 && publishedId !== null) {
      publishedByAsignatura.set(asignaturaId, publishedId);
    }
  });
  ...
  return items.filter((item) => {
    const payload = typeof item?.toJSON === 'function' ? item.toJSON() : item;
    const asignaturaId = Number(payload?.Asignatura?.id ?? payload?.asignatura_id);
    const publishedId = publishedByAsignatura.get(asignaturaId);
...
const ensureStudentCanAccessPublishedMiniproyecto = async (req, miniproyecto) => {
  ...
  const Asignatura = miniproyecto?.Asignatura || await AsignaturaModel.findByPk(miniproyecto.asignatura_id, {
    attributes: ['id', 'miniproyecto_publicado_id'],
  });
  const publishedId = getPublishedMiniproyectoId(Asignatura);
...
const clearPublishedMiniproyectoIfMatches = async ({ asignaturaId, miniproyectoId, transaction }) => {
  ...
  if (getPublishedMiniproyectoId(Asignatura) === Number(miniproyectoId)) {
    await Asignatura.update({ miniproyecto_publicado_id: null }, { transaction });
  }
};

const syncPublishedMiniproyectoOnAsignaturaChange = async ({ previousasignaturaId, nextasignaturaId, miniproyectoId, transaction }) => {
  if (Number(previousasignaturaId) === Number(nextasignaturaId)) {
    return;
  }

  await clearPublishedMiniproyectoIfMatches({
    asignaturaId: previousasignaturaId,
    miniproyectoId,
    transaction,
  });
};
...
exports.create = async (req, res) => {
  try {
    ...
    await validarRelaciones(req.body.tipo_actividad_id, req.body.asignatura_id);

    if (req.docenteAsignaturaId && parseInt(req.body.asignatura_id, 10) !== parseInt(req.docenteAsignaturaId, 10)) {
      return res.status(403).json({ error: "Acceso denegado: asignatura fuera de tu alcance" });
    }

    const configurablePayload = normalizeConfigurableMiniproyectoPayload(req.body.respuesta_miniproyecto);
    if (configurablePayload) {
      await validateConfigurableExercises(req.body.asignatura_id, configurablePayload.exercises.length > 0 ? configurablePayload : configurablePayload.exerciseIds);
      if (configurablePayload.chatbot?.enabled) {
        await validateSelectedChatbot(req.body.asignatura_id, configurablePayload.chatbot.chatbotId);
      }
    ...
    try {
      ...
      if (configurablePayload) {
        ...
        await syncSelectedChatbot({
          asignaturaId: Number(req.body.asignatura_id),
...
exports.update = async (req, res) => {
  try {
    ...
    if (req.docenteAsignaturaId && parseInt(miniproyecto.asignatura_id, 10) !== parseInt(req.docenteAsignaturaId, 10)) {
      return res.status(403).json({ error: "Acceso denegado: asignatura fuera de tu alcance" });
    }

    if (req.docenteAsignaturaId && req.body.asignatura_id !== undefined && parseInt(req.body.asignatura_id, 10) !== parseInt(req.docenteAsignaturaId, 10)) {
      return res.status(403).json({ error: "Acceso denegado: asignatura fuera de tu alcance" });
    }

    const targetasignaturaId = req.body.asignatura_id !== undefined ? Number(req.body.asignatura_id) : Number(miniproyecto.asignatura_id);
    ...
    if (configurablePayload) {
      await validateConfigurableExercises(targetasignaturaId, configurablePayload.exercises.length > 0 ? configurablePayload : configurablePayload.exerciseIds);
      if (configurablePayload.chatbot?.enabled) {
        await validateSelectedChatbot(targetasignaturaId, configurablePayload.chatbot.chatbotId, miniproyecto.id);
      }
    }
    ...
    try {
      ...
      const miniproyectoPayload = {
        ...(req.body.asignatura_id !== undefined && { asignatura_id: req.body.asignatura_id }),
      ...
      if (configurablePayload) {
        const selectedChatbotId = configurablePayload.chatbot?.enabled ? configurablePayload.chatbot.chatbotId : null;
        await syncSelectedChatbot({
          asignaturaId: targetasignaturaId,
      ...
      await syncPublishedMiniproyectoOnAsignaturaChange({
        previousasignaturaId: Number(miniproyecto.asignatura_id),
        nextasignaturaId: targetasignaturaId,
        miniproyectoId: Number(req.params.id),
        transaction: t,
      });
...
exports.findAll = async (req, res) => {
  try {
    ...
    if (req.docenteAsignaturaId) {
      where.asignatura_id = parseInt(req.docenteAsignaturaId, 10);
    } else if (asignatura_id !== undefined) {
      const parsedasignaturaId = parseInt(asignatura_id, 10);
      if (isNaN(parsedasignaturaId)) {
        return res.status(400).json({ error: 'asignatura_id debe ser un número válido' });
      }
      where.asignatura_id = parsedasignaturaId;
    }

    const data = await Miniproyecto.findAll({
      where,
      attributes: { exclude: ['asignatura_id'] },
      include: [
        { model: AsignaturaModel },
        {
          model: Chatbot,
          as: 'chatbots',
          attributes: ['id', 'nombre', 'tipo', 'estado', 'asignatura_id', 'miniproyecto_id']
        },
        { 
          model: Actividad,
...
exports.findOne = async (req, res) => {
  try {
    const attributes = req.docenteAsignaturaId ? undefined : { exclude: ['asignatura_id'] };
    ...
    if (req.docenteAsignaturaId && parseInt(data.asignatura_id, 10) !== parseInt(req.docenteAsignaturaId, 10)) {
      return res.status(403).json({ error: "Acceso denegado: asignatura fuera de tu alcance" });
    }

    if (req.docenteAsignaturaId) {
      const payload = serializeMiniproyectoSelection(data);
      delete payload.asignatura_id;
...
exports.delete = async (req, res) => {
  try {
    ...
    if (req.docenteAsignaturaId) {
      if (parseInt(miniproyecto.asignatura_id, 10) !== parseInt(req.docenteAsignaturaId, 10)) {
        return res.status(403).json({ error: "Acceso denegado: asignatura fuera de tu alcance" });
      }
    }
    ...
    try {
      ...
      await clearPublishedMiniproyectoIfMatches({
        asignaturaId: Number(miniproyecto.asignatura_id),
...
exports.updateStudentPublication = async (req, res) => {
  try {
    const miniproyecto = await Miniproyecto.findByPk(req.params.id, {
      include: [{ model: Actividad }, { model: AsignaturaModel }],
    });
    ...
    const t = await sequelize.transaction();
    try {
      const Asignatura = await AsignaturaModel.findByPk(miniproyecto.asignatura_id, {
        attributes: ['id', 'miniproyecto_publicado_id'],
        transaction: t,
      });
      ...
      if (shouldPublish) {
        await Asignatura.update({ miniproyecto_publicado_id: miniproyecto.id }, { transaction: t });
      } else if (publishedId === Number(miniproyecto.id)) {
        await Asignatura.update({ miniproyecto_publicado_id: null }, { transaction: t });
      }
    ...
    return res.json({
      message: shouldPublish
        ? 'Miniproyecto asignado al asignatura para estudiantes.'
        : 'El miniproyecto ya no está asignado como miniproyecto del asignatura.',
      asignatura_id: Number(miniproyecto.asignatura_id),
      miniproyecto_id: Number(miniproyecto.id),
      seleccionadoParaEstudiantes: shouldPublish,
    });
  } catch (err) {
    return res.status(err.status || 500).json({ message: err.message || 'No fue posible actualizar la asignación del miniproyecto del asignatura.' });
  }
};