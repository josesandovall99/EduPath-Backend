...
exports.enviarRespuestaEjercicio = async (req, res) => {
  try {
    ...
    // Bloquear nuevos intentos si ya está aprobado en el período actual
    const evaluacionAprobada = await Evaluacion.findOne({
      where: { estudiante_id, ejercicio_id: parseInt(ejercicioId, 10), estado: 'Aprobado', periodo_academico: periodoAcademico }
    });
    ...
    if (esCorrecta) {
      const evalWhere = { estudiante_id, ejercicio_id: parseInt(ejercicioId, 10), periodo_academico: periodoAcademico };
